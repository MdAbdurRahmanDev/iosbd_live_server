...
function addToCartDirect(id, redirectToCheckout, qty=null) {
    ...
    var product_price = $('#' + id + '-product_price').val();
    // alert(product_price);
    var quantity = 1;
    if(qty > 0){
        quantity = qty;
    ...
    var color = $('#' + id + '-color option:selected').val();
    var size = $('#' + id + '-size option:selected').val();
    var varient = $('#' + id + '-varient').val();

    // Get options if exists
    var options = [];
    if($('#' + id + '-choice_form').length) {
        options = $('#' + id + '-choice_form').serializeArray();
    }
...
<script>
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                @else
                    ...
                    $('#product_name').attr('href', '/product-details/'+data.product.slug);
                ...
                $('#product_price').val(data.product.regular_price);
    ...
    function addToCart() {
        ...
        for (var i = 1; i <= total_attributes; i++) {
            var attrValue = $('.attr_value_' + i).val();
            if (attrValue) {
                options.push({
                    name: 'attribute_ids[]',
                    value: $('#attribute_id_' + i).val()
                });
                options.push({
                    name: 'attribute_names[]',
                    value: $('#attribute_name_' + i).val()
                });
                options.push({
                    name: 'attribute_options[]',
                    value: attrValue
                });
                varient = attrValue; // Use the last attribute as variant
            }
        }
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                if(data && data != 'na'){
                    ...
                    if(data.qty <= 0){
                        $('#p_out_of_stock').removeClass('d-none');
                        $('#p_in_stock').addClass('d-none');
                    }else{
                        $('#p_out_of_stock').addClass('d-none');
                        $('#p_in_stock').removeClass('d-none');
                    }

                    if (data.image) {
                        var newImagePath = window.location.origin + '/' + data.image;

                        // Update main product image
                        $('#xzoom-default').attr("src", newImagePath);
                        $('#xzoom-default').attr("xoriginal", newImagePath);

                        // If you have a zoomed image somewhere else, update it too
                        $('#product_zoom_img').attr("src", newImagePath);
                        $('#product_zoom_img').attr("srcset", newImagePath);
                    }
                }
            }
        });
    ...
    function selectAttributeModal(id, i) {
        var value = $('#choice_option_attr_'+id).val();
        $('.attr_value_'+i).val(value);
        $('#attribute_check_'+i).val(1);
    }
...
<script>
    $(document).ready(function () {
        $('#userDropdownToggle').on('click', function () {
            $('.user-account-content').toggleClass('show');
        });

        // Hide when clicked outside
        $(document).on('click', function (e) {
            if (!$(e.target).closest('.h-offer-icon').length) {
                $('.user-account-content').removeClass('show');
            }
        });
    });
</script>
