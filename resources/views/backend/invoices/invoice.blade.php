				...
				<tr>
					<td>
                        ...
                        <div style="display: inline; align-items:center">
                            <img align="" src="data:image/jpg;base64,{{base64_encode(file_get_contents(base_path('public/'.$logo)))}}" style="width: 5.5rem; margin-bottom:-3px!important;"><span style="color: #F7941E; font-size: 1.5rem;"></span>
	                ...
	                @foreach ($order->order_details as $key => $orderDetail)
		                @if ($orderDetail->product != null)
							<tr class="">
			                    ...
			                    <td class="text-right currency">{{ round($orderDetail->price*$orderDetail->qty) }}</td>
							</tr>
		                @endif
					@endforeach
	            </tbody>
					        ...
					        <table class="text-right sm-padding small strong">
						        <tbody>
							        ...
							        <tr>
							         ...
							         @if ($order->coupon)
							         <tr class="border-top">
							            <th class="gry-color text-left">Applied Coupon:</th>
							            <td class="currency"><i>{{ $order->coupon }}</i></td>
							        </tr>
							         @endif
