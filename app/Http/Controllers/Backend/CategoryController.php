...
class CategoryController extends Controller{
    ...
    public function store(Request $request)
    {
        ...
        $specifications = [];
        if (!empty($request->featured) && is_array($request->featured)) {
            $specification = new CategorySpecification;
            $specifications =  array_filter($request->featured);

            foreach ($specifications as $specification) {
                $CategorySpecification = new CategorySpecification;
                $CategorySpecification->category_id = $category->id;
                $CategorySpecification->name = trim($specification);
    ...
    public function edit($id)
    {
        ...
        return view('backend.category.edit',compact('category', 'categories','specifications'));
    } // end method
    ...
    public function update(Request $request, $id)
    {
       ...
       /* ======== Category Parent Id  ======= */
        ...
        // $specification_ids = [];
        // if (!empty($request->featured) && is_array($request->featured)) {
        //     $specification_names =  array_filter($request->featured);
            
        //     $specification_ids =  array_filter($request->specification_id);
        //     $specifications = CategorySpecification::where('category_id',$category->id)->get()->pluck('id')->toArray();
        //     $ids_to_delete = array_diff($specifications, $specification_ids);

        //     if (!empty($ids_to_delete)) {
        //         ProductSpecification::whereIn('specification_id',$ids_to_delete)->delete();
        //         CategorySpecification::whereIn('id', $ids_to_delete)->delete();
        //     }

        //     foreach ($specification_ids as $index => $id) {
        //         $spec = CategorySpecification::find($id);
        //         if ($spec) {
        //             $spec->name = trim($specification_names[$index]); 
        //             $spec->save();
        //         }
        //     }
        ...
        $specification_names = array_filter($request->featured ?? []); 
        $specification_ids   = $request->specification_id ?? []; // don't filter yet, we need indexes aligned
        
        // Get all existing specs for this category
        $existing_specs = CategorySpecification::where('category_id', $category->id)->pluck('id')->toArray();
        
        // Find which ones to delete (existing but not in request)
        $ids_to_delete = array_diff($existing_specs, array_filter($specification_ids));
        ...
        // Loop over submitted specifications
        foreach ($specification_names as $index => $name) {
            $id = $specification_ids[$index] ?? null;
    ...
    public function destroy(Request $request)
    {
        if(!demo_mode()){
            ...
            // dd($category);
            $ids_to_delete = CategorySpecification::where('category_id',$category->id)->get()->pluck('id')->toArray();
            ProductSpecification::whereIn('specification_id',$ids_to_delete)->delete();
            CategorySpecification::whereIn('id', $ids_to_delete)->delete();
