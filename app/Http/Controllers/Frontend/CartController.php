...
class CartController extends Controller
{
    ...
    {
        try {
            $rowId = $request->rowId; // Get rowId

            if (!$rowId) {
                return response()->json(['success' => false, 'message' => 'Cart rowId is missing.'], 400);
            }

            // Check if item exists before removing
            $cartItem = Cart::get($rowId);
            if (!$cartItem) {
                return response()->json(['success' => false, 'message' => "The cart does not contain rowId $rowId."], 400);
            }
