...
function process_order($value){
    return $order = Order::where('delivery_status', $value)->count() ?? 0;
}

function return_request()
{
    return ReturnRequest::count() ?? 0;
}
function yesterday_total_order() {
    return Order::whereDate('created_at', Carbon::yesterday())->count() ?? 0;
}
function yesterday_total_sales() {
    return Order::whereRaw("DATE(created_at) = CURDATE() - INTERVAL 1 DAY")->sum('grand_total');
}
function total_complaints() {
    if (!Schema::hasColumn('user_messages', 'complain')) {
        return 0;
    }

    return UserMessage::sum('complain');
}
